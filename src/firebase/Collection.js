...
export default class {
    ...
    constructor () {
      Object.values(this.constructor.fields).forEach((fieldType) => {
        ...
        if (isClass) {
          ...
          funcs.forEach((func) => {
            Object.assign(this, {
              [func]: fieldType.prototype[func].bind(this)
            })
          })
          ...
          funcs.forEach((func) => {
            Object.assign(this, {
              [func]: types[fName].prototype[func].bind(this)
            })
          })
        }
      })
    ...
    async setData (id, data, doc = null, fetchStorageLink = true) {
      this.empty = false
      this.id = id
    ...
    static async getDocument (id) {
      ...
      try {
        ...
        const data = doc.data()
        const instance = new this()
        await instance.setData(doc.id, data, doc, false)
      ...
      try {
        ...
        if (snap.empty) {
          ...
          return await Promise.all(utils.parseDocs(snap.docs).map(async (datum, idx) => {
            const instance = new this()
            await instance.setData(datum.id, datum, snap.docs[idx], false)
            return instance
          }))
        }
      } catch (err) {
